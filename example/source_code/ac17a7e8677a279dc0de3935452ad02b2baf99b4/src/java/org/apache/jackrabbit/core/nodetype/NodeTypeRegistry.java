org apach jackrabbit core nodetyp
org apach common collect refer map referencemap
org apach jackrabbit core intern internalvalu
org apach jackrabbit core namespac registri impl namespaceregistryimpl
org apach jackrabbit core qname
org apach jackrabbit core file system filesystem
org apach jackrabbit core file system except filesystemexcept
org apach jackrabbit core file system resourc filesystemresourc
org apach log4j logger
javax jcr properti type propertytyp
javax jcr repositori except repositoryexcept
javax jcr nodetyp constraint violat except constraintviolationexcept
javax jcr nodetyp node type except nosuchnodetypeexcept
javax jcr version parent version action onparentversionact
java except ioexcept
java input stream inputstream
java output stream outputstream
java print stream printstream
java util
code nodetyperegistri code
node type registri nodetyperegistri
logger log logger logger getlogg node type registri nodetyperegistri
node type name
unstructur
qname unstructur unstructur
qname namespac registri impl namespaceregistryimpl uri uri unstructur
base
qname base base
qname namespac registri impl namespaceregistryimpl uri uri base
hierarchynod
qname hierarchynod hierarchynod
qname namespac registri impl namespaceregistryimpl uri uri hierarchynod
mimeresourc
qname mime resourc mime resourc
qname namespac registri impl namespaceregistryimpl uri uri mimeresourc
queri
qname queri queri
qname namespac registri impl namespaceregistryimpl uri uri queri
mix referenc
qname mix referenc mix referenc
qname namespac registri impl namespaceregistryimpl mix uri mix uri referenc
mix lockabl
qname mix lockabl mix lockabl
qname namespac registri impl namespaceregistryimpl mix uri mix uri lockabl
mix version
qname mix version mix version
qname namespac registri impl namespaceregistryimpl mix uri mix uri version
versionhistori
qname version histori version histori
qname namespac registri impl namespaceregistryimpl uri uri versionhistori
version
qname version version
qname namespac registri impl namespaceregistryimpl uri uri version
frozenversionablechild
qname frozen version child frozen version child
qname namespac registri impl namespaceregistryimpl uri uri frozenversionablechild
jcr primarytyp
qname jcr primari type jcr primari type
qname namespac registri impl namespaceregistryimpl jcr uri jcr uri primarytyp
string builtin nodetyp resourc path builtin nodetyp resourc path
org apach jackrabbit core nodetyp builtin nodetyp xml
string custom nodetyp resourc custom nodetyp resourc custom nodetyp xml
file system node type registr persist
file system filesystem store ntstore
resourc hold custom node type definit repres
node repositori need order make registr
persist
file system resourc filesystemresourc custom node type resourc customnodetypesresourc
persist node type definit built custom node type
node type def store nodetypedefstor built def builtinntdef
node type def store nodetypedefstor custom def customntdef
cach pre built aggreg node type
effect node type cach effectivenodetypecach ent cach entcach
map node type name node type definit
hash map hashmap regist def registeredntdef
definit root node
child node def childnodedef root node def rootnodedef
map id' properti definit
hash map hashmap prop def propdef
map id' node definit
hash map hashmap node def nodedef
namespac registri resolv prefix namespac uri'
serial node type definit
namespac registri impl namespaceregistryimpl reg nsreg
listen soft refer
map listen
collect synchron map synchronizedmap refer map referencemap refer map referencemap weak refer map referencemap weak
creat code nodetyperegistri code
param nsreg
param ntstore
return code nodetyperegistri code object
throw repositoryexcept
node type registri nodetyperegistri creat namespac registri impl namespaceregistryimpl reg nsreg file system filesystem store ntstore
repositori except repositoryexcept
node type registri nodetyperegistri mgr ntmgr node type registri nodetyperegistri reg nsreg store ntstore
mgr ntmgr
privat constructor
param nsreg
param ntstore
throw repositoryexcept
node type registri nodetyperegistri namespac registri impl namespaceregistryimpl reg nsreg file system filesystem store ntstore
repositori except repositoryexcept
reg nsreg reg nsreg
store ntstore store ntstore
custom node type resourc customnodetypesresourc file system resourc filesystemresourc store ntstore custom nodetyp resourc custom nodetyp resourc
make path resourc exist
custom node type resourc customnodetypesresourc exist
custom node type resourc customnodetypesresourc make parent dir makeparentdir
file system except filesystemexcept fse
string error intern error invalid resourc custom node type resourc customnodetypesresourc path getpath
log error error fse
repositori except repositoryexcept error fse
ent cach entcach effect node type cach effectivenodetypecach
regist def registeredntdef hash map hashmap
prop def propdef hash map hashmap
node def nodedef hash map hashmap
setup definit root node
root node def rootnodedef creat root node def createrootnodedef
node def nodedef put node def nodedefid root node def rootnodedef root node def rootnodedef
load regist pre defin built node type
built def builtinntdef node type def store nodetypedefstor
input stream inputstream
class getclass class loader getclassload resourc stream getresourceasstream builtin nodetyp resourc path builtin nodetyp resourc path
built def builtinntdef load
intern regist internalregist built def builtinntdef
except ioexcept ioe
string error intern error fail read built node type definit store builtin nodetyp resourc path builtin nodetyp resourc path
log error error ioe
repositori except repositoryexcept error ioe
invalid node type def except invalidnodetypedefexcept intd
string error intern error invalid built node type definit store builtin nodetyp resourc path builtin nodetyp resourc path
log error error intd
repositori except repositoryexcept error intd
close
except ioexcept ioe
ignor
load regist custom node type
custom def customntdef node type def store nodetypedefstor
custom node type resourc customnodetypesresourc exist
custom node type resourc customnodetypesresourc input stream getinputstream
file system except filesystemexcept fse
string error intern error fail access custom node type definit store custom node type resourc customnodetypesresourc path getpath
log error error fse
repositori except repositoryexcept error fse
log info custom node type definit found
custom def customntdef load
intern regist internalregist custom def customntdef
except ioexcept ioe
string error intern error fail read custom node type definit store custom node type resourc customnodetypesresourc path getpath
log error error ioe
repositori except repositoryexcept error ioe
invalid node type def except invalidnodetypedefexcept intd
string error intern error invalid custom node type definit store custom node type resourc customnodetypesresourc path getpath
log error error intd
repositori except repositoryexcept error intd
close
except ioexcept ioe
ignor
child node def childnodedef creat root node def createrootnodedef
child node def childnodedef def child node def childnodedef
fixm fake declar node type
def set declar node type setdeclaringnodetyp qname namespac registri impl namespaceregistryimpl default uri default uri
def set requir primari type setrequiredprimarytyp qname base base
def set default primari type setdefaultprimarytyp unstructur unstructur
def set mandatori setmandatori
def set protect setprotect
def set parent version setonparentvers parent version action onparentversionact version
def set primari item setprimaryitem
def set sib setallowsamenamesib
def set auto creat setautocr
def
valid regist collect code nodetypedef code
object code invalidnodetypedefexcept code thrown
valid contain code nodetypedef code object fail
note case except thrown node type
successfulli regist
param ntdef collect code nodetypedef code object
throw invalidnodetypedefexcept
throw repositoryexcept
registernodetyp
intern regist internalregist collect def ntdef
invalid node type def except invalidnodetypedefexcept repositori except repositoryexcept
arrai list arraylist list arrai list arraylist def ntdef
iter definit definit
unresolv unregist depend error occur
count int count number regist nt' iter
list size count
count
iter iter list iter
iter hasnext
node type def nodetypedef ntd node type def nodetypedef iter
check definit unresolv depend
regist def registeredntdef kei set keyset containsal ntd depend getdepend
regist
intern regist internalregist ntd
remov list
iter remov
increas count
count
list size
string buffer stringbuff msg string buffer stringbuff
msg append node type regist unresolv depend
iter iter list iter
iter hasnext
msg append node type def nodetypedef iter getnam
msg append
log error msg string tostr
invalid node type def except invalidnodetypedefexcept msg string tostr
effect node type effectivenodetyp intern regist internalregist node type def nodetypedef ntd
invalid node type def except invalidnodetypedefexcept repositori except repositoryexcept
qname ntd getnam
regist def registeredntdef kei containskei
string msg exist
log error msg
invalid node type def except invalidnodetypedefexcept msg
effect node type effectivenodetyp ent valid node type def validatenodetypedef ntd
store effect node type instanc
ent cach entcach put ent
regist clone node type definit
ntd node type def nodetypedef ntd clone
clone support except clonenotsupportedexcept
log fatal intern error
intern error internalerror messag getmessag
regist def registeredntdef put ntd
store poperti child node definit node type
prop def propdef pda ntd properti def getpropertydef
pda length
prop def propdef def pda
prop def propdefid prop def propdefid def
prop def propdef put def
child node def childnodedef nda ntd child node def getchildnodedef
nda length
child node def childnodedef def nda
node def nodedefid node def nodedefid def
node def nodedef put def
ent
add code nodetyperegistrylisten code
param listen listen inform registr
node type
add listen addlisten node type registri listen nodetyperegistrylisten listen
listen kei containskei listen
listen put listen listen
remov code nodetyperegistrylisten code
param listen exist listen
remov listen removelisten node type registri listen nodetyperegistrylisten listen
listen remov listen
notifi listen node type code ntname code regist
notifi regist notifyregist qname ntname
copi listen arrai avoid concurrentmodificationexcept
node type registri listen nodetyperegistrylisten node type registri listen nodetyperegistrylisten listen size
iter iter listen valu iter
cnt
iter hasnext
cnt node type registri listen nodetyperegistrylisten iter
length
node type regist nodetyperegist ntname
notifi listen node type code ntname code unregist
notifi unregist notifyunregist qname ntname
copi listen arrai avoid concurrentmodificationexcept
node type registri listen nodetyperegistrylisten node type registri listen nodetyperegistrylisten listen size
iter iter listen valu iter
cnt
iter hasnext
cnt node type registri listen nodetyperegistrylisten iter
length
node type unregist nodetypeunregist ntname
effect node type effectivenodetyp valid node type def validatenodetypedef node type def nodetypedef ntd
invalid node type def except invalidnodetypedefexcept repositori except repositoryexcept
effect merg resolv node type result
node type definit
effect node type final creat definit
verifi check conflict case
creat earlier stage valid
child node definit
effect node type effectivenodetyp ent
qname ntd getnam
string msg
log error msg
invalid node type def except invalidnodetypedefexcept msg
regist def registeredntdef kei containskei
string msg exist
log error msg
invalid node type def except invalidnodetypedefexcept msg
valid supertyp
qname supertyp ntd supertyp getsupertyp
supertyp supertyp length
supertyp length
simpl check infinit recurs
trap recurs deeper inherit level
equal supertyp
string msg invalid supertyp supertyp infinit recurs
log error msg
invalid node type def except invalidnodetypedefexcept msg
regist def registeredntdef kei containskei supertyp
string msg invalid supertyp supertyp
log error msg
invalid node type def except invalidnodetypedefexcept msg
check circular inherit chain
'a' extend 'b' extend 'a'
stack inherit chain inheritancechain stack
inherit chain inheritancechain push
check circular inherit checkforcircularinherit supertyp inherit chain inheritancechain
note infinit recurs inherit automat
check call geteffectivenodetyp
imposs regist node type definit
refer supertyp regist
build effect merg resolv node type supertyp
check conflict
supertyp supertyp length
effect node type effectivenodetyp est build effect node type buildeffectivenodetyp supertyp
make primari type base extend base
ntd mixin ismixin base base equal ntd getnam
est includ node type includesnodetyp base base
string msg primari node type base directli indirectli deriv base
log error msg
invalid node type def except invalidnodetypedefexcept msg
node type conflict except nodetypeconflictexcept ntce
string msg fail valid supertyp
log error msg ntce
invalid node type def except invalidnodetypedefexcept msg ntce
node type except nosuchnodetypeexcept nsnte
string msg fail valid supertyp
log error msg nsnte
invalid node type def except invalidnodetypedefexcept msg nsnte
supertyp mixin type base
ntd mixin ismixin base base equal ntd getnam
string msg primari node type base directli indirectli deriv base
log error msg
invalid node type def except invalidnodetypedefexcept msg
child item def childitemdef primari item primaryitem
valid properti definit
prop def propdef pda ntd properti def getpropertydef
pda length
prop def propdef pda
check primari item flag
primari item isprimaryitem
defin residu definesresidu
string msg primari item
log error msg
invalid node type def except invalidnodetypedefexcept msg
primari item primaryitem
string msg primari item
log error msg
invalid node type def except invalidnodetypedefexcept msg
primari item primaryitem
check auto creat properti
defin residu definesresidu auto creat isautocr
string msg auto creat properti
log error msg
invalid node type def except invalidnodetypedefexcept msg
check default valu
make type consist requir properti type
intern internalvalu def val defval default valu getdefaultvalu
def val defval def val defval length
req type reqtyp requir type getrequiredtyp
def val defval length
req type reqtyp properti type propertytyp undefin
req type reqtyp def val defval type gettyp
def val defval type gettyp req type reqtyp
string msg type default consist requir properti type
log error msg
invalid node type def except invalidnodetypedefexcept msg
todo check auto creat properti default valu system gener valu
check default valu satisfi constraint
constraint valueconstraint constraint constraint getvalueconstraint
constraint constraint length
def val defval def val defval length
check constraint
def val defval length
constraint
satisfi
constraint violat except constraintviolationexcept cve
constraint length
constraint check def val defval
constraint satisfi
satisfi
constraint violat except constraintviolationexcept
cve
satisfi
report except encount
string msg default properti
defin residu definesresidu getnam string tostr
satisfi constraint
log error msg cve
invalid node type def except invalidnodetypedefexcept msg cve
referenceconstraint
node type regist notabl
except node type regist
requir type getrequiredtyp properti type propertytyp refer
constraint length
refer constraint referenceconstraint refer constraint referenceconstraint constraint
qname ntname node type getnodetypenam
equal ntname regist def registeredntdef kei containskei ntname
string msg invalid refer constraint
ntname unknown node type properti definit
defin residu definesresidu getnam string tostr
log error msg
invalid node type def except invalidnodetypedefexcept msg
valid child node definit
child node def childnodedef cnda ntd child node def getchildnodedef
cnda length
child node def childnodedef cnd cnda
check primari item flag
cnd primari item isprimaryitem
cnd defin residu definesresidu
string msg primari item
log error msg
invalid node type def except invalidnodetypedefexcept msg
primari item primaryitem
string msg primari item
log error msg
invalid node type def except invalidnodetypedefexcept msg
primari item primaryitem cnd
check auto creat child node
cnd defin residu definesresidu cnd auto creat isautocr
string msg auto creat child node
log error msg
invalid node type def except invalidnodetypedefexcept msg
check default primari type
qname dpt cnd default primari type getdefaultprimarytyp
refer referencetoself
effect node type effectivenodetyp default ent default
dpt
check node type specifi default primari type
equal dpt
refer referencetoself
default primari type regist notabl
except node type regist
equal dpt regist def registeredntdef kei containskei dpt
string msg invalid default primari type dpt
childnod definit cnd getnam
log error msg
invalid node type def except invalidnodetypedefexcept msg
build effect merg resolv node type
default primari type check conflict
refer referencetoself
default ent default effect node type geteffectivenodetyp dpt
default primari type ident node
type regist instanti
'manually'
ent effect node type effectivenodetyp creat ntd
default ent default ent
cnd auto creat isautocr
check circular default primari type
auto creat child node node type 'a' defin
auto creat child node default primari type 'a'
stack defin definingnt stack
defin definingnt push
check circular node auto creation checkforcircularnodeautocr default ent default defin definingnt
node type conflict except nodetypeconflictexcept ntce
string msg fail valid default primari type
log error msg ntce
invalid node type def except invalidnodetypedefexcept msg ntce
node type except nosuchnodetypeexcept nsnte
string msg fail valid default primari type
log error msg nsnte
invalid node type def except invalidnodetypedefexcept msg nsnte
check requir primari type
qname req type reqtyp cnd requir primari type getrequiredprimarytyp
req type reqtyp req type reqtyp length
req type reqtyp length
qname rpt req type reqtyp
refer referencetoself
check node type specifi requir
primari type
equal rpt
refer referencetoself
requir primari type regist
notabl except node type regist
equal rpt regist def registeredntdef kei containskei rpt
string msg invalid requir primari type rpt
log error msg
invalid node type def except invalidnodetypedefexcept msg
check default primari type satisfi requir
primari type constraint
default ent default default ent default includ node type includesnodetyp rpt
string msg default primari type satisfi requir primari type constraint rpt
log error msg
invalid node type def except invalidnodetypedefexcept msg
build effect merg resolv node type
requir primari type constraint check conflict
refer referencetoself
effect node type geteffectivenodetyp rpt
requir primari type ident
node type regist
instanti 'manually'
ent
ent effect node type effectivenodetyp creat ntd
node type conflict except nodetypeconflictexcept ntce
string msg fail valid requir primari type constraint
log error msg ntce
invalid node type def except invalidnodetypedefexcept msg ntce
node type except nosuchnodetypeexcept nsnte
string msg fail valid requir primari type constraint
log error msg nsnte
invalid node type def except invalidnodetypedefexcept msg nsnte
build effect merg resolv node type
node type definit potenti detect
conflict problem
ent
ent effect node type effectivenodetyp creat ntd
node type conflict except nodetypeconflictexcept ntce
string msg fail resolv node type definit
log error msg ntce
invalid node type def except invalidnodetypedefexcept msg ntce
node type except nosuchnodetypeexcept nsnte
string msg fail resolv node type definit
log error msg nsnte
invalid node type def except invalidnodetypedefexcept msg nsnte
ent
return
qname regist node type getregisterednodetyp
qname regist def registeredntdef kei set keyset arrai toarrai qname regist def registeredntdef size
return
child node def childnodedef root node def getrootnodedef
root node def rootnodedef
param ntname
return
effect node type effectivenodetyp effect node type geteffectivenodetyp qname ntname
node type except nosuchnodetypeexcept
weight kei weightedkei kei weight kei weightedkei qname ntname
ent cach entcach kei
ent cach entcach kei
node type except nosuchnodetypeexcept ntname string tostr
param ntname
return
effect node type effectivenodetyp build effect node type buildeffectivenodetyp qname name ntname
node type conflict except nodetypeconflictexcept node type except nosuchnodetypeexcept
make singl node type regist
name ntname length
regist def registeredntdef kei containskei name ntname
node type except nosuchnodetypeexcept name ntname string tostr
weight kei weightedkei kei weight kei weightedkei name ntname
check aggreg build
ent cach entcach kei
ent cach entcach kei
build aggreg
effect node type effectivenodetyp result
build list 'best' exist aggreg
arrai list arraylist tmp result tmpresult arrai list arraylist
kei size
check built aggreg
ent cach entcach kei
tmp result tmpresult add ent cach entcach kei
subtract result temporari kei
'empty'
kei kei subtract kei
walk list exist aggreg sort 'weight'
aggreg cost build
found result foundsubresult
iter iter ent cach entcach kei
iter hasnext
weight kei weightedkei weight kei weightedkei iter
check exist aggreg 'subset'
kei
tmp result tmpresult add ent cach entcach
subtract result temporari kei
kei kei subtract
found result foundsubresult
found result foundsubresult
match aggreg found
build aggreg remain node type iter
qname remaind kei arrai toarrai
remaind length
effect node type effectivenodetyp ent
ent effect node type effectivenodetyp creat remaind
store effect node type
ent cach entcach put ent
result
result ent
result result merg ent
store intermedi result aggreg
ent cach entcach put result
add aggreg remain node type result list
tmp result tmpresult add result
merg aggreg effect node type
tmp result tmpresult size
result
result effect node type effectivenodetyp tmp result tmpresult
result result merg effect node type effectivenodetyp tmp result tmpresult
store intermedi result
ent cach entcach put result
result
check circular inherit checkforcircularinherit qname supertyp stack inherit chain inheritancechain
invalid node type def except invalidnodetypedefexcept repositori except repositoryexcept
supertyp length
qname supertyp
po inherit chain inheritancechain index lastindexof
po
string buffer stringbuff buf string buffer stringbuff
inherit chain inheritancechain size
po
buf append
buf append inherit chain inheritancechain
buf append extend
buf append
buf append
invalid node type def except invalidnodetypedefexcept circular inherit detect buf string tostr
qname sta node type def getnodetypedef supertyp getsupertyp
sta sta length
check recurs
inherit chain inheritancechain push
check circular inherit checkforcircularinherit sta inherit chain inheritancechain
inherit chain inheritancechain pop
node type except nosuchnodetypeexcept nsnte
string msg unknown supertyp
log error msg nsnte
invalid node type def except invalidnodetypedefexcept msg nsnte
check circular node auto creation checkforcircularnodeautocr effect node type effectivenodetyp child node ent childnod stack defin parent definingparentnt
invalid node type def except invalidnodetypedefexcept
check circular default node type auto creat child node
node type 'a' defin auto creat child node default node type 'a'
qname child node childnod child node ent childnod node type getallnodetyp
child node childnod length
qname child node childnod
po defin parent definingparentnt index lastindexof
po
string buffer stringbuff buf string buffer stringbuff
defin parent definingparentnt size
po
buf append
buf append node type
buf append defin parent definingparentnt
buf append defin auto creat child node default
buf append
buf append node type
buf append
invalid node type def except invalidnodetypedefexcept circular node auto creation detect buf string tostr
child node def childnodedef node def nodedef child node ent childnod auto creat node def getautocreatenodedef
node def nodedef length
qname dnt node def nodedef default primari type getdefaultprimarytyp
qname defin definingnt node def nodedef declar node type getdeclaringnodetyp
dnt
check recurs
defin parent definingparentnt push defin definingnt
check circular node auto creation checkforcircularnodeautocr effect node type geteffectivenodetyp dnt defin parent definingparentnt
defin parent definingparentnt pop
node type except nosuchnodetypeexcept nsnte
string msg defin definingnt defin invalid default node type child node node def nodedef getnam
log error msg nsnte
invalid node type def except invalidnodetypedefexcept msg nsnte
valid code nodetypedef code return
regist code effectivenodetyp code instanc
valid includ check
supertyp exist regist
inherit graph circular
aggreg supertyp result conflict
ambigu
definit auto creat properti
default valu properti definit satisfi constraint
properti definit
definit auto creat child node
default node type child node definit exist
regist
aggreg default node type child node definit
result conflict ambigu
definit auto creat child node default
node type lead infinit child node creation
node type 'a' defin auto creat child node default
node type 'a'
node type constraint child node definit
exist regist
aggreg node type constraint
child node definit result conflict ambigu
default node type child node definit satisfi
node type constraint child node definit
param ntd definit node type
return code effectivenodetyp code instanc
throw invalidnodetypedefexcept
throw repositoryexcept
effect node type effectivenodetyp regist node type registernodetyp node type def nodetypedef ntd
invalid node type def except invalidnodetypedefexcept repositori except repositoryexcept
valid regist node type definit
effect node type effectivenodetyp ent intern regist internalregist ntd
persist node type definit
custom def customntdef add ntd
output stream outputstream
custom node type resourc customnodetypesresourc output stream getoutputstream
custom def customntdef store reg nsreg
except ioexcept ioe
string error intern error fail write custom node type definit custom node type resourc customnodetypesresourc path getpath
log error error ioe
repositori except repositoryexcept error ioe
file system except filesystemexcept fse
string error intern error fail write custom node type definit custom node type resourc customnodetypesresourc path getpath
log error error fse
repositori except repositoryexcept error fse
close
except ioexcept ioe
ignor
notifi listen
notifi regist notifyregist ntd getnam
ent
code link registernodetyp nodetypedef code
collect code nodetypedef code regist
method regist set node type
depend
note case except thrown node type
successfulli regist
param ntdef collect code nodetypedef code
throw invalidnodetypedefexcept
throw repositoryexcept
regist node type registernodetyp collect def ntdef
invalid node type def except invalidnodetypedefexcept repositori except repositoryexcept
except thrown internalregist collect
repositori except repositoryexcept
invalid node type def except invalidnodetypedefexcept intd
store name regist node type proceed
hash set hashset name oldntnam hash set hashset regist def registeredntdef kei set keyset
valid regist node type definit
intern regist internalregist def ntdef
repositori except repositoryexcept
store except thrown
invalid node type def except invalidnodetypedefexcept
store except thrown
intd
build set name regist node type
potenti subset ntdef except
thrown
hash set hashset name newntnam hash set hashset regist def registeredntdef kei set keyset
name newntnam remov removeal name oldntnam
name newntnam size
persist node type definit
iter iter name newntnam iter iter hasnext
qname ntname qname iter
custom def customntdef add node type def nodetypedef regist def registeredntdef ntname
output stream outputstream
custom node type resourc customnodetypesresourc output stream getoutputstream
custom def customntdef store reg nsreg
except ioexcept ioe
string error intern error fail write custom node type definit custom node type resourc customnodetypesresourc path getpath
log error error ioe
repositori except repositoryexcept error ioe
file system except filesystemexcept fse
string error intern error fail write custom node type definit custom node type resourc customnodetypesresourc path getpath
log error error fse
repositori except repositoryexcept error fse
close
except ioexcept ioe
ignor
notifi listen
iter iter name newntnam iter iter hasnext
qname ntname qname iter
notifi regist notifyregist ntname
throw except
intd
intd
param
throw nosuchnodetypeexcept
throw repositoryexcept
unregist node type unregisternodetyp qname
node type except nosuchnodetypeexcept repositori except repositoryexcept
regist def registeredntdef kei containskei
node type except nosuchnodetypeexcept string tostr
built def builtinntdef
repositori except repositoryexcept string tostr unregist built node type
check node type depend
node type
set depend dependentnt depend node type getdependentnodetyp
depend dependentnt size
string buffer stringbuff msg string buffer stringbuff
msg append remov node type referenc
iter iter depend dependentnt iter
iter hasnext
msg append qname iter
msg append
repositori except repositoryexcept msg string tostr
todo
appli deep lock root node workspac altern
put repositori 'singl user' mode
check node type referenc node
repositori
remov node type referenc
throw except
check absolut order guarante
integr repositori content
throw except implement properli
referenc isreferenc
referenc isreferenc
repositori except repositoryexcept implement
node type def nodetypedef ntd node type def nodetypedef regist def registeredntdef
regist def registeredntdef remov
remov effect node type aggreg cach
ent cach entcach remov qname
remov poperti child node definit
prop def propdef pda ntd properti def getpropertydef
pda length
prop def propdefid prop def propdefid pda
prop def propdef remov
child node def childnodedef nda ntd child node def getchildnodedef
nda length
node def nodedefid node def nodedefid nda
node def nodedef remov
persist remov node type definit
custom def customntdef remov
output stream outputstream
custom node type resourc customnodetypesresourc output stream getoutputstream
custom def customntdef store reg nsreg
except ioexcept ioe
string error intern error fail write custom node type definit custom node type resourc customnodetypesresourc path getpath
log error error ioe
repositori except repositoryexcept error ioe
file system except filesystemexcept fse
string error intern error fail write custom node type definit custom node type resourc customnodetypesresourc path getpath
log error error fse
repositori except repositoryexcept error fse
close
except ioexcept ioe
ignor
todo remov node type aggreg depend node type
notifi listen
notifi unregist notifyunregist
param ntd
return
throw nosuchnodetypeexcept
throw invalidnodetypedefexcept
throw repositoryexcept
effect node type effectivenodetyp reregist node type reregisternodetyp node type def nodetypedef ntd
node type except nosuchnodetypeexcept invalid node type def except invalidnodetypedefexcept
repositori except repositoryexcept
qname ntd getnam
regist def registeredntdef kei containskei
node type except nosuchnodetypeexcept string tostr
built def builtinntdef
repositori except repositoryexcept string tostr reregist built node type
valid node type definit
effect node type effectivenodetyp ent valid node type def validatenodetypedef ntd
collect name node type depend
node type
set depend dependentnt depend node type getdependentnodetyp
todo
determin type chang build diff current
definit
chang trivial effect current content
properti defint chang
singl valu multi valu continu step
continu step
appli deep lock root node workspac altern
put repositori 'exclusive' 'singl user' mode
check node type node type
depend node type referenc node
repositori
check appli affect node violat
exist node type constraint
regist node type definit updat cach
notifi listen registr
appli persist affect node updat
definit id'
unregisternodetyp
return registernodetyp ntd
repositori except repositoryexcept implement
return name regist node type
depend node type
param nodetypenam
return set node type code qname code
throw nosuchnodetypeexcept
set depend node type getdependentnodetyp qname node type nodetypenam
node type except nosuchnodetypeexcept
regist def registeredntdef kei containskei node type nodetypenam
node type except nosuchnodetypeexcept node type nodetypenam string tostr
collect name node type depend
node type
hash set hashset name hash set hashset
iter iter regist def registeredntdef valu iter
iter hasnext
node type def nodetypedef ntd node type def nodetypedef iter
ntd depend getdepend node type nodetypenam
name add ntd getnam
name
param nodetypenam
return
throw nosuchnodetypeexcept
node type def nodetypedef node type def getnodetypedef qname node type nodetypenam node type except nosuchnodetypeexcept
regist def registeredntdef kei containskei node type nodetypenam
node type except nosuchnodetypeexcept node type nodetypenam string tostr
node type def nodetypedef def node type def nodetypedef regist def registeredntdef node type nodetypenam
return clone make mess 'real' definit
node type def nodetypedef def clone
clone support except clonenotsupportedexcept
log fatal intern error
intern error internalerror messag getmessag
param nodetypenam
return
regist isregist qname node type nodetypenam
regist def registeredntdef kei containskei node type nodetypenam
param
return
child node def childnodedef node def getnodedef node def nodedefid
child node def childnodedef def child node def childnodedef node def nodedef
def
return clone make mess 'real' definit
child node def childnodedef def clone
clone support except clonenotsupportedexcept
log fatal intern error
intern error internalerror messag getmessag
param
return
prop def propdef prop def getpropdef prop def propdefid
prop def propdef def prop def propdef prop def propdef
def
return clone make mess 'real' definit
prop def propdef def clone
clone support except clonenotsupportedexcept
log fatal intern error
intern error internalerror messag getmessag
diagnost
dump state code nodetypemanag code instanc
param
throw repositoryexcept
dump print stream printstream repositori except repositoryexcept
println nodetypemanag
println
println regist nodetyp
println
iter iter regist def registeredntdef valu iter
iter hasnext
node type def nodetypedef ntd node type def nodetypedef iter
println ntd getnam
qname supertyp ntd supertyp getsupertyp
println tsupertyp
supertyp length
println supertyp
println tmixin ntd mixin ismixin
println torderablechildnod ntd order child node hasorderablechildnod
prop def propdef ntd properti def getpropertydef
length
print tpropertydef
println declar declar node type getdeclaringnodetyp prop def propdefid
println tname defin residu definesresidu getnam string tostr
string type requir type getrequiredtyp null properti type propertytyp namefromvalu requir type getrequiredtyp
println trequiredtyp type
constraint valueconstraint vca constraint getvalueconstraint
string buffer stringbuff constraint string buffer stringbuff
vca
constraint append null
vca length
constraint length
constraint append
constraint append vca definit getdefinit
println tvalueconstraint constraint string tostr
intern internalvalu def val defval default valu getdefaultvalu
string buffer stringbuff default valu defaultvalu string buffer stringbuff
def val defval
default valu defaultvalu append null
def val defval length
default valu defaultvalu length
default valu defaultvalu append
default valu defaultvalu append def val defval string tostr
println tdefaultvalu default valu defaultvalu string tostr
println tautocr auto creat isautocr
println tmandatori mandatori ismandatori
println tonvers parent version action onparentversionact namefromvalu parent version getonparentvers
println tprotect protect isprotect
println tprimaryitem primari item isprimaryitem
println tmultipl multipl ismultipl
child node def childnodedef ntd child node def getchildnodedef
length
print tnodedef
println declar declar node type getdeclaringnodetyp node def nodedefid
println tname defin residu definesresidu getnam string tostr
qname req primari type reqprimarytyp requir primari type getrequiredprimarytyp
req primari type reqprimarytyp req primari type reqprimarytyp length
req primari type reqprimarytyp length
print trequiredprimarytyp req primari type reqprimarytyp
qname def primari type defprimarytyp default primari type getdefaultprimarytyp
def primari type defprimarytyp
print tdefaultprimarytyp def primari type defprimarytyp
println tautocr auto creat isautocr
println tmandatori mandatori ismandatori
println tonvers parent version action onparentversionact namefromvalu parent version getonparentvers
println tprotect protect isprotect
println tprimaryitem primari item isprimaryitem
println tallowsamenamesib sib allowsamenamesib
println
ent cach entcach dump
class
code weightedkei code uniqu identifi
combin aggreg node type
weight indic cost involv build
aggreg aggreg multipl complex node type deep
inherit tree costli build valid agregg
simpl node type properti definit
simpl accur approxim weight
number explicitli aggreg node type ignor inherit
complex involv node type approxim
number explicitli implicitli note
inherit aggreg aggreg node type
accur weight definit effici
build aggreg
import note weight part kei
consid code hashcod code
code equal object code method affect order
code weightedkei code instanc
code link compareto object code inform
assum aggreg node type name
kei weight simpl
approxim
weight kei weightedkei compar
set node type name sort ascend order
tree set treeset set
weight
param ntname
weight kei weightedkei qname name ntname
name ntname name ntname length
param ntname
param weight
weight kei weightedkei qname name ntname weight
weight weight
set tree set treeset
name ntname length
add sort set
set add name ntname
param ntname
weight kei weightedkei collect name ntname
name ntname name ntname size
param ntname
param weight
weight kei weightedkei collect name ntname weight
weight weight
set tree set treeset name ntname
kei string represent sort set
kei set entri
return string represent sort set
abstractcollect tostr
string kei getkei
set string tostr
return
weight getweight
weight
size
set size
iter iter
collect unmodifi sort set unmodifiablesortedset set iter
set set getset
collect unmodifi sort set unmodifiablesortedset set
qname arrai toarrai
qname set arrai toarrai qname set size
weight kei weightedkei kei otherkei
set containsal kei otherkei set getset
weight kei weightedkei subtract weight kei weightedkei kei otherkei
set tmp set set clone
tmp remov removeal kei otherkei set getset
weight kei weightedkei tmp
result sort order descend weight ascend kei
string represent sort set
param
return
compar compareto object
weight kei weightedkei weight kei weightedkei
weight getweight weight getweight
weight getweight weight getweight
kei getkei compar compareto kei getkei
hash code hashcod
ignor weight
iter set iter
hasnext
object obj
obj obj hash code hashcod
equal object obj
obj
obj weight kei weightedkei
weight kei weightedkei weight kei weightedkei obj
ignor weight
set equal set
string string tostr
set string tostr weight
code efectivenodetypecach code
effect node type cach effectivenodetypecach
order set kei
tree set treeset sort kei sortedkei
cach pre build aggreg node type
hash map hashmap aggreg
effect node type cach effectivenodetypecach
sort kei sortedkei tree set treeset
aggreg hash map hashmap
put effect node type effectivenodetyp ent
defin weight total number includ node type
aggreg inherit
weight ent node type getallnodetyp length
effect node type identifi list merg
aggreg node type
weight kei weightedkei weight kei weightedkei ent merg node type getmergednodetyp weight
aggreg put ent
sort kei sortedkei add
qname name ntname
aggreg kei containskei weight kei weightedkei name ntname
weight kei weightedkei kei
aggreg kei containskei kei
effect node type effectivenodetyp qname name ntname
effect node type effectivenodetyp aggreg weight kei weightedkei name ntname
effect node type effectivenodetyp weight kei weightedkei kei
effect node type effectivenodetyp aggreg kei
effect node type effectivenodetyp remov qname name ntname
remov weight kei weightedkei name ntname
effect node type effectivenodetyp remov weight kei weightedkei kei
effect node type effectivenodetyp remov effect node type effectivenodetyp aggreg remov kei
remov
remov index entri
fixm simpli call treeset remov kei entri
sortedkei weight
found
iter iter sort kei sortedkei iter
iter hasnext
weight kei weightedkei weight kei weightedkei iter
weightedkei equal object ignor weight
kei equal
sort kei sortedkei remov
remov
return iter kei order return kei
descend weight
ascend kei uniqu identifi aggreg
nodetyperegistri weightedkei compareto
iter kei
sort kei sortedkei iter
diagnost
dump state code effectivenodetypecach code instanc
param
throw repositoryexcept
dump print stream printstream repositori except repositoryexcept
println effectivenodetypecach
println
println effectivenodetyp cach
println
iter iter sort kei sortedkei iter
iter hasnext
weight kei weightedkei weight kei weightedkei iter
effectivenodetyp ent effectivenodetyp aggreg
println
