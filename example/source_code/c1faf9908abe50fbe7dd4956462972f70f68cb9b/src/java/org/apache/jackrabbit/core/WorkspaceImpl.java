...
flag privat internalcopi method
copi
clone
...
state mgr statemgr
dump state code workspaceimpl code instanc
diagnost purpos
param
...
check node state satisfi constraint impli
...
child node def childnodedef def reg ntreg node def getnodedef node state nodest definit getdefinitionid
...
string msg hier mgr hiermgr safe jcr path safegetjcrpath node state nodest getid
...
type valu type gettyp
constraint violat except constraintviolationexcept hier mgr hiermgr safe jcr path safegetjcrpath prop state propstat getid
inconsist type
...
check ad ad child node call code nodenam code
node type code nodetypenam code parent node allow
current context
param parentst
param nodenam
param nodetypenam
...
lock manag getlockmanag check lock checklock parent path parentpath session
version statu
...
item found except itemnotfoundexcept hier mgr hiermgr safe jcr path safegetjcrpath parent state parentst getid
...
access deni except accessdeniedexcept hier mgr hiermgr safe jcr path safegetjcrpath parent state parentst getid
...
constraint violat except constraintviolationexcept hier mgr hiermgr safe jcr path safegetjcrpath parent state parentst getid
...
hier mgr hiermgr safe jcr path safegetjcrpath parent state parentst getid
...
check remov target node allow current
context
param targetst
param option bit wise or' flag check
perform combin constant
...
path found except pathnotfoundexcept hier mgr hiermgr safe jcr path safegetjcrpath target targetid
...
access deni except accessdeniedexcept hier mgr hiermgr safe jcr path safegetjcrpath target targetid
allow remov node
item found except itemnotfoundexcept inf
string msg intern error fail check access right
hier mgr hiermgr safe jcr path safegetjcrpath target targetid
log debug msg
...
constraint violat except constraintviolationexcept hier mgr hiermgr safe jcr path safegetjcrpath target targetid
...
ref refer hasrefer
referenti integr except referentialintegrityexcept hier mgr hiermgr safe jcr path safegetjcrpath target targetid
remov node refer
item state except itemstateexcept is
...
child node
precondit state manag workspac
edit mode
param targetst
param parentuuid
throw repositoryexcept error occur
remov node state removenodest node state nodest target state targetst string parent uuid parentuuid
repositori except repositoryexcept
...
check exist remov
check remov node checkremovenod node state nodest check access check access check lock check lock
check version check version
remov child node recurs
remov node state removenodest node state nodest target state targetst uuid getuuid
item state except itemstateexcept is
string msg intern error fail retriev state
node nodeid
log debug msg
...
orphan
destroi target state
state mgr statemgr destroi target state targetst
store target state
state mgr statemgr store target state targetst
recurs copi node state includ properti
child node
precondit state manag code code workspac
edit mode
param srcstate
param parentuuid
param srcstatemgr
...
node uuid exist
item exist except itemexistsexcept hier mgr hiermgr safe jcr path safegetjcrpath
clone remov exist clone remov exist
referenc
referenc node creat uuid
uuid uuid random uuid randomuuid string tostr uuid uuid randomuuid tostr creat version uuid
...
remov exist
remov node state removenodest exist state existingst exist state existingst parent uuid getparentuuid
...
src state mgr srcstatemgr src access mgr srcaccessmgr flag
store child node
...
access manag sourc workspac session
access manag accessmanag src access mgr srcaccessmgr
...
src wsp srcwsp item state manag getitemstatemanag src access mgr srcaccessmgr flag
add parent
...
remov exist removeexist clone remov exist clone remov exist clone
...
saniti check sanitycheck
intra workspac copi
intern copi internalcopi src ab path srcabspath dest ab path destabspath copi
inheritdoc
...
workspac impl workspaceimpl src wsp srcwsp workspac impl workspaceimpl src session srcsession workspac getworkspac
cross workspac copi
intern copi internalcopi src ab path srcabspath src wsp srcwsp dest ab path destabspath copi
src session srcsession
session anymor logout
src session srcsession logout
inheritdoc
...
check remov node checkremovenod target state targetst check access check access check lock check lock
check version check version check constraint check constraint
check add node checkaddnod dest parent state destparentst dest destnam getnam
target state targetst node type getnodetypenam check access check access check lock check lock
check version check version check constraint check constraint
