org apach jackrabbit core
org apach jackrabbit api jackrabbit workspac jackrabbitworkspac
org apach jackrabbit api jsr283 observ event journal eventjourn
org apach jackrabbit core config workspac config workspaceconfig
org apach jackrabbit core lock lock manag lockmanag
org apach jackrabbit core observ event state collect eventstatecollect
org apach jackrabbit core observ event state collect factori eventstatecollectionfactori
org apach jackrabbit core observ observ manag impl observationmanagerimpl
org apach jackrabbit core observ event journal impl eventjournalimpl
org apach jackrabbit core observ event filter eventfilt
org apach jackrabbit core queri queri manag impl querymanagerimpl
org apach jackrabbit core state local item state manag localitemstatemanag
org apach jackrabbit core state share item state manag shareditemstatemanag
org apach jackrabbit core version date version selector dateversionselector
org apach jackrabbit core version version impl versionimpl
org apach jackrabbit core version version selector versionselector
org apach jackrabbit core xml import handler importhandl
org apach jackrabbit core xml import
org apach jackrabbit core xml workspac import workspaceimport
org apach jackrabbit core cluster cluster node clusternod
org apach jackrabbit core secur princip admin princip adminprincip
org apach jackrabbit common abstract workspac abstractworkspac
org apach jackrabbit spi common convers except nameexcept
org apach jackrabbit spi path
org slf4j logger
org slf4j logger factori loggerfactori
org xml sax content handler contenthandl
org xml sax input sourc inputsourc
javax jcr access deni except accessdeniedexcept
javax jcr invalid item state except invaliditemstateexcept
javax jcr item exist except itemexistsexcept
javax jcr item found except itemnotfoundexcept
javax jcr namespac registri namespaceregistri
javax jcr workspac except nosuchworkspaceexcept
javax jcr path found except pathnotfoundexcept
javax jcr repositori except repositoryexcept
javax jcr session
javax jcr unsupport repositori oper except unsupportedrepositoryoperationexcept
javax jcr lock lock except lockexcept
javax jcr nodetyp constraint violat except constraintviolationexcept
javax jcr nodetyp node type manag nodetypemanag
javax jcr observ observ manag observationmanag
javax jcr queri queri manag querymanag
javax jcr version version
javax jcr version version except versionexcept
javax jcr version version histori versionhistori
javax secur auth subject
java util hash map hashmap
java util iter
code workspaceimpl code
workspac impl workspaceimpl abstract workspac abstractworkspac
jackrabbit workspac jackrabbitworkspac event state collect factori eventstatecollectionfactori
logger log logger factori loggerfactori logger getlogg workspac impl workspaceimpl
configur code workspac code
workspac config workspaceconfig wsp config wspconfig
repositori creat workspac instanc
repositori impl repositoryimpl rep
persist state mgr workspac repres
code workspac code instanc
local item state manag localitemstatemanag state mgr statemgr
hierarchi mgr reflect persist state
isol transient made
session
cach hierarchi manag cachinghierarchymanag hier mgr hiermgr
code observationmanag code instanc session
observ manag impl observationmanagerimpl ob mgr obsmgr
code querymanag code code workspac code
queri manag impl querymanagerimpl queri manag querymanag
session acquir code workspac code
session impl sessionimpl session
code lockmanag code code workspac code
lock manag lockmanag lock mgr lockmgr
protect constructor
param wspconfig workspac configur
param statemgr share item state manag
param rep repositori
param session session
workspac impl workspaceimpl workspac config workspaceconfig wsp config wspconfig
share item state manag shareditemstatemanag state mgr statemgr repositori impl repositoryimpl rep
session impl sessionimpl session
wsp config wspconfig wsp config wspconfig
rep rep
state mgr statemgr creat item state manag createitemstatemanag state mgr statemgr
hier mgr hiermgr
cach hierarchi manag cachinghierarchymanag rep root node getrootnodeid state mgr statemgr
state mgr statemgr add listen addlisten hier mgr hiermgr
session session
hierarchi manag reflect workspac state
isol transient made
session
return hierarchi manag workspac
hierarchi manag hierarchymanag hierarchi manag gethierarchymanag
hier mgr hiermgr
return item state manag workspac
repres code workspaceimpl code instanc
return item state manag workspac
local item state manag localitemstatemanag item state manag getitemstatemanag
state mgr statemgr
dispos code workspaceimpl code free resourc
dispos
ob mgr obsmgr
ob mgr obsmgr dispos
ob mgr obsmgr
state mgr statemgr dispos
perform saniti check workspac session
throw repositoryexcept workspac render invalid
reason
saniti check sanitycheck repositori except repositoryexcept
check session statu
session saniti check sanitycheck
jsr method
creat code workspac code code code
initi code clone code content workspac
code srcworkspac code semant method equival
creat workspac manual clone code srcworkspac code
method assist implement optim
subsequ code node updat code code node merg code call
workspac sourc
workspac access code login code
throw code accessdeniedexcept code session
code workspac code object acquir permiss
creat workspac
throw code unsupportedrepositoryoperationexcept code repositori
support creation workspac
code repositoryexcept code thrown error occur
param code string code workspac
param srcworkspac workspac workspac clone
throw accessdeniedexcept session
code workspac code object acquir permiss
creat workspac
throw unsupportedrepositoryoperationexcept repositori
support creation workspac
throw repositoryexcept error occur
jcr
creat workspac createworkspac string string src workspac srcworkspac
access deni except accessdeniedexcept unsupport repositori oper except unsupportedrepositoryoperationexcept
repositori except repositoryexcept
check state instanc
saniti check sanitycheck
session creat workspac createworkspac
session impl sessionimpl tmp session tmpsession
creat temporari session workspac current subject
tmp session tmpsession rep creat session createsess session subject getsubject
workspac impl workspaceimpl wsp newwsp workspac impl workspaceimpl tmp session tmpsession workspac getworkspac
wsp newwsp clone src workspac srcworkspac
tmp session tmpsession
temporari session anymor logout
tmp session tmpsession logout
delet workspac code code
repositori delet content
throw code accessdeniedexcept code session
code workspac code object acquir permiss
remov workspac
throw code unsupportedrepositoryoperationexcept code
repositori support remov workspac
param code string code workspac delet
throw accessdeniedexcept session
code workspac code object acquir permiss
remov workspac
throw unsupportedrepositoryoperationexcept
repositori support remov workspac
throw repositoryexcept error occur
jcr
delet workspac deleteworkspac string access deni except accessdeniedexcept
unsupport repositori oper except unsupportedrepositoryoperationexcept repositori except repositoryexcept
check workspac exist throw nosuchworkspaceexcept
rep workspac info getworkspaceinfo
todo implement deleteworkspac
unsupport repositori oper except unsupportedrepositoryoperationexcept implement
jackrabbitworkspac jsr method
creat code workspac code
code code workspac empti mean
root node
workspac access code login code
throw code accessdeniedexcept code session
code workspac code object acquir permiss
creat workspac
throw code unsupportedrepositoryoperationexcept code repositori
support creation workspac
code repositoryexcept code thrown error occur
param code string code workspac
throw accessdeniedexcept session
code workspac code object acquir permiss
creat workspac
throw unsupportedrepositoryoperationexcept repositori
support creation workspac
throw repositoryexcept error occur
jcr
creat workspac createworkspac string
access deni except accessdeniedexcept
unsupport repositori oper except unsupportedrepositoryoperationexcept
repositori except repositoryexcept
check state instanc
saniti check sanitycheck
session creat workspac createworkspac
jackrabbitworkspac
creat workspac workspac configur
templat
param workspacenam workspac
param configtempl configur templat workspac
throw accessdeniedexcept current session allow
creat workspac
throw repositoryexcept workspac
exist error occur
getaccessibleworkspacenam
creat workspac createworkspac string workspac workspacenam input sourc inputsourc config templat configtempl
access deni except accessdeniedexcept repositori except repositoryexcept
check state instanc
saniti check sanitycheck
session creat workspac createworkspac workspac workspacenam config templat configtempl
return configur workspac
return workspac configur
workspac config workspaceconfig config getconfig
wsp config wspconfig
param srcabspath
param srcwsp
param destabspath
param flag
code copi code
code clone code
code clone remov exist code
return path node posit
throw constraintviolationexcept
throw accessdeniedexcept
throw versionexcept
throw pathnotfoundexcept
throw itemexistsexcept
throw lockexcept
throw repositoryexcept
string intern copi internalcopi string src ab path srcabspath
workspac impl workspaceimpl src wsp srcwsp
string dest ab path destabspath
flag
constraint violat except constraintviolationexcept access deni except accessdeniedexcept
version except versionexcept path found except pathnotfoundexcept item exist except itemexistsexcept
lock except lockexcept repositori except repositoryexcept
path src path srcpath
src path srcpath session path getqpath src ab path srcabspath normal path getnormalizedpath
except nameexcept
string msg invalid path src ab path srcabspath
log debug msg
repositori except repositoryexcept msg
src path srcpath absolut isabsolut
repositori except repositoryexcept absolut path src ab path srcabspath
path dest path destpath
dest path destpath session path getqpath dest ab path destabspath normal path getnormalizedpath
except nameexcept
string msg invalid path dest ab path destabspath
log debug msg
repositori except repositoryexcept msg
dest path destpath absolut isabsolut
repositori except repositoryexcept absolut path dest ab path destabspath
batch item oper batcheditemoper op batch item oper batcheditemoper
state mgr statemgr rep node type registri getnodetyperegistri session lock manag getlockmanag
session hier mgr hiermgr
op edit
illeg state except illegalstateexcept
string msg unabl start edit oper
log debug msg
repositori except repositoryexcept msg
succeed
node nodeid op copi src path srcpath src wsp srcwsp item state manag getitemstatemanag
src wsp srcwsp hierarchi manag gethierarchymanag
session impl sessionimpl src wsp srcwsp session getsess access manag getaccessmanag
dest path destpath flag
op updat
succeed
session jcr path getjcrpath hier mgr hiermgr path getpath
succeed
updat oper fail cancel modif
op cancel
handl clone insid workspac support
shareabl node
link clone
param srcabspath sourc path
param destabspath destin path
return path node posit
throw constraintviolationexcept
throw accessdeniedexcept
throw versionexcept
throw pathnotfoundexcept
throw itemexistsexcept
throw lockexcept
throw repositoryexcept
string intern clone internalclon string src ab path srcabspath string dest ab path destabspath
constraint violat except constraintviolationexcept access deni except accessdeniedexcept
version except versionexcept path found except pathnotfoundexcept item exist except itemexistsexcept
lock except lockexcept repositori except repositoryexcept
path src path srcpath
src path srcpath session path getqpath src ab path srcabspath normal path getnormalizedpath
except nameexcept
string msg invalid path src ab path srcabspath
log debug msg
repositori except repositoryexcept msg
src path srcpath absolut isabsolut
repositori except repositoryexcept absolut path src ab path srcabspath
path dest path destpath
dest path destpath session path getqpath dest ab path destabspath normal path getnormalizedpath
except nameexcept
string msg invalid path dest ab path destabspath
log debug msg
repositori except repositoryexcept msg
dest path destpath absolut isabsolut
repositori except repositoryexcept absolut path dest ab path destabspath
batch item oper batcheditemoper op batch item oper batcheditemoper
state mgr statemgr rep node type registri getnodetyperegistri session lock manag getlockmanag
session hier mgr hiermgr
op edit
illeg state except illegalstateexcept
string msg unabl start edit oper
log debug msg
repositori except repositoryexcept msg
succeed
item itemid op clone src path srcpath dest path destpath
op updat
succeed
session jcr path getjcrpath hier mgr hiermgr path getpath
succeed
updat oper fail cancel modif
op cancel
return lock manag workspac creat
instanc
return lock manag workspac
throw repositoryexcept error occur
lock manag lockmanag lock manag getlockmanag repositori except repositoryexcept
check state instanc
saniti check sanitycheck
lock mgr lockmgr
lock mgr lockmgr rep lock manag getlockmanag wsp config wspconfig getnam
lock mgr lockmgr
workspac
inheritdoc
string getnam
wsp config wspconfig getnam
inheritdoc
session session getsess
session
inheritdoc
namespac registri namespaceregistri namespac registri getnamespaceregistri repositori except repositoryexcept
check state instanc
saniti check sanitycheck
rep namespac registri getnamespaceregistri
inheritdoc
node type manag nodetypemanag node type manag getnodetypemanag repositori except repositoryexcept
check state instanc
saniti check sanitycheck
session node type manag getnodetypemanag
inheritdoc
clone string src workspac srcworkspac string src ab path srcabspath
string dest ab path destabspath remov exist removeexist
workspac except nosuchworkspaceexcept constraint violat except constraintviolationexcept
version except versionexcept access deni except accessdeniedexcept path found except pathnotfoundexcept
item exist except itemexistsexcept lock except lockexcept repositori except repositoryexcept
check state instanc
saniti check sanitycheck
check workspac
getnam equal src workspac srcworkspac
clone workspac allow mix shareabl node
removeexist fals
remov exist removeexist
intern clone internalclon src ab path srcabspath dest ab path destabspath
current workspac
string msg src workspac srcworkspac illeg workspac current
log debug msg
repositori except repositoryexcept msg
check author workspac
session access manag getaccessmanag access canaccess src workspac srcworkspac
access deni except accessdeniedexcept author access src workspac srcworkspac
clone pull subtre srcabspath srcworkspac
'this' workspac destabspath
session impl sessionimpl src session srcsession
creat session workspac current subject
throw nosuchworkspaceexcept accessdeniedexcept
src session srcsession rep creat session createsess session subject getsubject src workspac srcworkspac
workspac impl workspaceimpl src wsp srcwsp workspac impl workspaceimpl src session srcsession workspac getworkspac
cross workspac copi
mode batch item oper batcheditemoper clone
remov exist removeexist
mode batch item oper batcheditemoper clone remov exist clone remov exist
intern copi internalcopi src ab path srcabspath src wsp srcwsp dest ab path destabspath mode
src session srcsession
session anymor logout
src session srcsession logout
inheritdoc
copi string src ab path srcabspath string dest ab path destabspath
constraint violat except constraintviolationexcept version except versionexcept
access deni except accessdeniedexcept path found except pathnotfoundexcept item exist except itemexistsexcept
lock except lockexcept repositori except repositoryexcept
check state instanc
saniti check sanitycheck
intra workspac copi
intern copi internalcopi src ab path srcabspath dest ab path destabspath batch item oper batcheditemoper copi
inheritdoc
copi string src workspac srcworkspac string src ab path srcabspath string dest ab path destabspath
workspac except nosuchworkspaceexcept constraint violat except constraintviolationexcept
version except versionexcept access deni except accessdeniedexcept path found except pathnotfoundexcept
item exist except itemexistsexcept lock except lockexcept repositori except repositoryexcept
check state instanc
saniti check sanitycheck
check workspac
getnam equal src workspac srcworkspac
current workspac deleg intra workspac copi method
copi src ab path srcabspath dest ab path destabspath
check author workspac
session access manag getaccessmanag access canaccess src workspac srcworkspac
access deni except accessdeniedexcept author access src workspac srcworkspac
copi pull subtre srcabspath srcworkspac
'this' workspac destabspath
session impl sessionimpl src session srcsession
creat session workspac current subject
throw nosuchworkspaceexcept accessdeniedexcept
src session srcsession rep creat session createsess session subject getsubject src workspac srcworkspac
workspac impl workspaceimpl src wsp srcwsp workspac impl workspaceimpl src session srcsession workspac getworkspac
cross workspac copi
intern copi internalcopi src ab path srcabspath src wsp srcwsp dest ab path destabspath batch item oper batcheditemoper copi
src session srcsession
session anymor logout
src session srcsession logout
inheritdoc
move string src ab path srcabspath string dest ab path destabspath
constraint violat except constraintviolationexcept version except versionexcept
access deni except accessdeniedexcept path found except pathnotfoundexcept item exist except itemexistsexcept
lock except lockexcept repositori except repositoryexcept
check state instanc
saniti check sanitycheck
intra workspac move
path src path srcpath
src path srcpath session path getqpath src ab path srcabspath normal path getnormalizedpath
except nameexcept
string msg invalid path src ab path srcabspath
log debug msg
repositori except repositoryexcept msg
src path srcpath absolut isabsolut
repositori except repositoryexcept absolut path src ab path srcabspath
path dest path destpath
dest path destpath session path getqpath dest ab path destabspath normal path getnormalizedpath
except nameexcept
string msg invalid path dest ab path destabspath
log debug msg
repositori except repositoryexcept msg
dest path destpath absolut isabsolut
repositori except repositoryexcept absolut path dest ab path destabspath
batch item oper batcheditemoper op batch item oper batcheditemoper
state mgr statemgr rep node type registri getnodetyperegistri session lock manag getlockmanag
session hier mgr hiermgr
op edit
illeg state except illegalstateexcept
string msg unabl start edit oper
log debug msg
repositori except repositoryexcept msg
succeed
node nodeid op move src path srcpath dest path destpath
op updat
succeed
succeed
updat oper fail cancel modif
op cancel
return observ manag session observ manag
lazili creat exist
return observ manag session
throw repositoryexcept repositori error occur
observ manag observationmanag observ manag getobservationmanag
repositori except repositoryexcept
check state instanc
saniti check sanitycheck
ob mgr obsmgr
ob mgr obsmgr observ manag impl observationmanagerimpl
rep observ dispatch getobservationdispatch wsp config wspconfig getnam
session session item manag getitemmanag
workspac except nosuchworkspaceexcept nswe
string msg intern error fail instanti observ manag
log debug msg
repositori except repositoryexcept msg nswe
ob mgr obsmgr
return event journal workspac event filter
pass criteria
param eventtyp combin event type constant encod bitmask
param abspath absolut path
param isdeep code boolean code
param uuid arrai uuid
param nodetypenam arrai node type name
return event journal repositori
throw unsupportedrepositoryoperationexcept repositori
support event journal cluster journal disabl
throw repositoryexcept error occur
event journal eventjourn event journal geteventjourn event type eventtyp
string ab path abspath
deep isdeep
string uuid
string node type nodetypenam
repositori except repositoryexcept
subject subject session impl sessionimpl session getsess subject getsubject
subject princip getprincip admin princip adminprincip empti isempti
repositori except repositoryexcept administr session
access eventjourn
cluster node clusternod cluster node clusternod rep cluster node getclusternod
cluster node clusternod
unsupport repositori oper except unsupportedrepositoryoperationexcept
observ manag impl observationmanagerimpl ob mgr obsmgr observ manag impl observationmanagerimpl session workspac getworkspac observ manag getobservationmanag
event filter eventfilt filter ob mgr obsmgr creat event filter createeventfilt event type eventtyp ab path abspath
deep isdeep uuid node type nodetypenam
event journal impl eventjournalimpl filter cluster node clusternod journal getjourn
cluster node clusternod getid
inheritdoc
queri manag querymanag queri manag getquerymanag repositori except repositoryexcept
check state instanc
saniti check sanitycheck
queri manag querymanag
search manag searchmanag search manag searchmanag
search manag searchmanag rep search manag getsearchmanag wsp config wspconfig getnam
search manag searchmanag
string msg search manag configur workspac
log debug msg
repositori except repositoryexcept msg
workspac except nosuchworkspaceexcept nswe
string msg intern error fail instanti queri manag
log debug msg
repositori except repositoryexcept msg nswe
queri manag querymanag queri manag impl querymanagerimpl session session item manag getitemmanag search manag searchmanag
queri manag querymanag
inheritdoc
restor version version remov exist removeexist
item exist except itemexistsexcept unsupport repositori oper except unsupportedrepositoryoperationexcept
version except versionexcept lock except lockexcept invalid item state except invaliditemstateexcept
repositori except repositoryexcept
todo perform restor oper direct node state
check state instanc
saniti check sanitycheck
add version map version restor
hash map hashmap restor torestor hash map hashmap
version length
version impl versionimpl version impl versionimpl version
version histori versionhistori histori getcontaininghistori
check collis
restor torestor kei containskei uuid getuuid
version except versionexcept unabl restor version version histori
restor torestor put uuid getuuid
creat version selector set version
version selector versionselector vsel version selector versionselector
version select version histori versionhistori version histori versionhistori repositori except repositoryexcept
select version
version version restor torestor version histori versionhistori uuid getuuid
select latest
date version selector dateversionselector select date selectbyd version histori versionhistori
check pend
session pend haspendingchang
string msg unabl restor version session pend
log debug msg
invalid item state except invaliditemstateexcept msg
restor version node
num restor numrestor
restor torestor size
version restor
iter iter restor torestor valu iter
iter hasnext
version impl versionimpl version impl versionimpl iter
node impl nodeimpl node node impl nodeimpl session node uuid getnodebyuuid frozen node getfrozennod frozen uuid getfrozenuuid
restor node intern restor internalrestor vsel remov exist removeexist
remov restor version set
restor length
restor torestor remov restor histori getcontaininghistori uuid getuuid
num restor numrestor restor length
item found except itemnotfoundexcept
ignor
restor
num restor numrestor
version except versionexcept unabl restor version
exist version node workspac
version except versionexcept unabl restor version
exist version node parent
repositori except repositoryexcept
revert session
log error revert appli restor
session refresh
repositori except repositoryexcept
ignor
session save
inheritdoc
string access workspac name getaccessibleworkspacenam repositori except repositoryexcept
check state instanc
saniti check sanitycheck
session workspac name getworkspacenam
inheritdoc
content handler contenthandl import content handler getimportcontenthandl string parent ab path parentabspath
uuid behavior uuidbehavior
path found except pathnotfoundexcept constraint violat except constraintviolationexcept
version except versionexcept lock except lockexcept repositori except repositoryexcept
check state instanc
saniti check sanitycheck
path parent path parentpath
parent path parentpath session path getqpath parent ab path parentabspath normal path getnormalizedpath
except nameexcept
string msg invalid path parent ab path parentabspath
log debug msg
repositori except repositoryexcept msg
parent path parentpath absolut isabsolut
repositori except repositoryexcept absolut path parent ab path parentabspath
import import workspac import workspaceimport parent path parentpath
rep node type registri getnodetyperegistri uuid behavior uuidbehavior
import handler importhandl import session
creat persist item state manag top share item
state manag overridden subclass
param share share item state manag
return local item state manag
local item state manag localitemstatemanag creat item state manag createitemstatemanag share item state manag shareditemstatemanag share
local item state manag localitemstatemanag share rep item state cach factori getitemstatecachefactori
eventstatecollectionfactori
inheritdoc
implement object forward link obsmgr
creation lazi
event state collect eventstatecollect creat event state collect createeventstatecollect
repositori except repositoryexcept
observ manag impl observationmanagerimpl observ manag getobservationmanag creat event state collect createeventstatecollect
