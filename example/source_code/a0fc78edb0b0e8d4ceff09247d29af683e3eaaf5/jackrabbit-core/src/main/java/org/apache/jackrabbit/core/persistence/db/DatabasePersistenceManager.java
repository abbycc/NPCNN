org apach jackrabbit core persist
org apach jackrabbit core node nodeid
org apach jackrabbit core properti propertyid
org apach jackrabbit core file system filesystem
org apach jackrabbit core local local file system localfilesystem
org apach jackrabbit core persist abstract persist manag abstractpersistencemanag
org apach jackrabbit core persist context pmcontext
org apach jackrabbit core persist util blob store blobstor
org apach jackrabbit core persist util file system blob store filesystemblobstor
org apach jackrabbit core persist util serial
org apach jackrabbit core state chang log changelog
org apach jackrabbit core state item state itemst
org apach jackrabbit core state item state except itemstateexcept
org apach jackrabbit core state item state except nosuchitemstateexcept
org apach jackrabbit core state node refer noderefer
org apach jackrabbit core state node refer nodereferencesid
org apach jackrabbit core state node state nodest
org apach jackrabbit core state properti state propertyst
org apach jackrabbit core blob file blobfilevalu
org apach jackrabbit core intern internalvalu
org apach jackrabbit util text
org slf4j logger
org slf4j logger factori loggerfactori
javax jcr properti type propertytyp
javax jcr repositori except repositoryexcept
java buffer reader bufferedread
java byte arrai input stream bytearrayinputstream
java byte arrai output stream bytearrayoutputstream
java file
java filter input stream filterinputstream
java except ioexcept
java input stream inputstream
java input stream reader inputstreamread
java sql connect
java sql databas meta data databasemetadata
java sql prepar statement preparedstat
java sql result set resultset
java sql sql except sqlexcept
java sql statement
java util hash map hashmap
java util iter
abstract base class databas persist manag class
common function databas persist manag subclass
differ databas connect acquir
subclass overrid link getconnect method return
configur databas connect
link simpledbpersistencemanag detail descript
configur option databas behaviour
databas persist manag databasepersistencemanag abstract persist manag abstractpersistencemanag
logger instanc
logger log logger factori loggerfactori logger getlogg databas persist manag databasepersistencemanag
string schema object prefix variabl schema object prefix variabl
schemaobjectprefix
initi
string schema
string schema object prefix schemaobjectprefix
extern blo externalblob
initi size buffer serial object
initi buffer size initi buffer size
jdbc connect
connect con
intern flag govern automat reconnect
attempt sqlexcept encount
auto reconnect autoreconnect
time sleep reconnect attempt
sleep reconnect sleep reconnect
map prepar statement kei sql stmt prepar stmt
hash map hashmap prepar statement preparedstat hash map hashmap
sql statement nodest manag
string node state insert sql nodestateinsertsql
string node state updat sql nodestateupdatesql
string node state select sql nodestateselectsql
string node state select exist sql nodestateselectexistsql
string node state delet sql nodestatedeletesql
sql statement propertyst manag
string properti state insert sql propertystateinsertsql
string properti state updat sql propertystateupdatesql
string properti state select sql propertystateselectsql
string properti state select exist sql propertystateselectexistsql
string properti state delet sql propertystatedeletesql
sql statement noderefer manag
string node refer insert sql nodereferenceinsertsql
string node refer updat sql nodereferenceupdatesql
string node refer select sql nodereferenceselectsql
string node refer select exist sql nodereferenceselectexistsql
string node refer delet sql nodereferencedeletesql
sql statement blob manag
code externalblob fals code
string blob insert sql blobinsertsql
string blob updat sql blobupdatesql
string blob select sql blobselectsql
string blob select exist sql blobselectexistsql
string blob delet sql blobdeletesql
file system blob data store
code externalblob true code
file system filesystem blob blobf
blobstor manag blob data file system
code externalblob true code
blob store blobstor blob store blobstor
creat code databasepersistencemanag code instanc
databas persist manag databasepersistencemanag
schema default
schema object prefix schemaobjectprefix
extern blo externalblob
initi
setter getter
string schema object prefix getschemaobjectprefix
schema object prefix schemaobjectprefix
set schema object prefix setschemaobjectprefix string schema object prefix schemaobjectprefix
make prefix uppercas
schema object prefix schemaobjectprefix schema object prefix schemaobjectprefix upper case touppercas
string schema getschema
schema
set schema setschema string schema
schema schema
extern blo isexternalblob
extern blo externalblob
set extern blo setexternalblob extern blo externalblob
extern blo externalblob extern blo externalblob
set extern blo setexternalblob string extern blo externalblob
extern blo externalblob boolean valueof extern blo externalblob boolean booleanvalu
persistencemanag
inheritdoc
init context pmcontext context except
initi
illeg state except illegalstateexcept initi
setup jdbc connect
init connect initconnect
databas meta data databasemetadata meta con meta data getmetadata
log info databas meta databas product getdatabaseproductnam meta databas product version getdatabaseproductvers
log info driver meta driver getdrivernam meta driver version getdriververs
sql except sqlexcept
log warn retriev databas driver version
make schemaobjectprefix consist legal charact
prepar schema object prefix prepareschemaobjectprefix
check schema object exist creat
check schema checkschema
build sql statement
build sql statement buildsqlstat
prepar statement
init prepar statement initpreparedstat
extern blo externalblob
store blob local file system directori
workspac home directori
local file system localfilesystem blob blobf local file system localfilesystem
blob blobf set root setroot file context home dir gethomedir blob
blob blobf init
blob blobf blob blobf
blob store blobstor file system blob store filesystemblobstor blob blobf
store blob
blob store blobstor blob store dbblobstor
initi
inheritdoc
close except
initi
illeg state except illegalstateexcept initi
close share prepar statement
iter prepar statement preparedstat valu iter hasnext
close statement closestat prepar statement preparedstat
prepar statement preparedstat clear
extern blo externalblob
close blob file system
blob blobf close
blob blobf
blob store blobstor
close jdbc connect
close connect closeconnect con
initi
inheritdoc
store chang log changelog chang log changelog
item state except itemstateexcept
temporarili disabl automat reconnect featur
persist atom
auto reconnect autoreconnect
item state except itemstateexcept is
number attempt store
trial
trial
store chang log changelog
item state except itemstateexcept
catch except fall
is
is is getcaus sql except sqlexcept
sqlexcept thrown
trial
reconnect
log warn store fail reconnect is getcaus
reconnect
reestablish connect reestablishconnect
give
is
reconnect fail proce error process
sqlexcept thrown
proce error process
is
store succeed commit
con commit
sql except sqlexcept
string msg commit chang log fail
log error msg
item state except itemstateexcept msg
store fail rollback
con rollback
sql except sqlexcept
string msg rollback chang log fail
log error msg
throw origin except
is
enabl automat reconnect featur
auto reconnect autoreconnect
inheritdoc
node state nodest load node nodeid
item state except nosuchitemstateexcept item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
node state select sql nodestateselectsql
result set resultset
input stream inputstream
statement stmt execut stmt executestmt node state select sql nodestateselectsql object string tostr
stmt result set getresultset
item state except nosuchitemstateexcept string tostr
binari stream getbinarystream
node state nodest state creat createnew
serial deseri state
state
except
item state except nosuchitemstateexcept
item state except nosuchitemstateexcept
string msg fail read node state
log error msg
item state except itemstateexcept msg
close stream closestream
close result set closeresultset
inheritdoc
properti state propertyst load properti propertyid
item state except nosuchitemstateexcept item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
properti state select sql propertystateselectsql
result set resultset
input stream inputstream
statement stmt execut stmt executestmt properti state select sql propertystateselectsql object string tostr
stmt result set getresultset
item state except nosuchitemstateexcept string tostr
binari stream getbinarystream
properti state propertyst state creat createnew
serial deseri state blob store blobstor
state
except
item state except nosuchitemstateexcept
item state except nosuchitemstateexcept
string msg fail read properti state
log error msg
item state except itemstateexcept msg
close stream closestream
close result set closeresultset
inheritdoc
method share code preparedstat code
execut strictli sequenti method synchron
persist manag instanc synchron
share statement method sychron share
statement synchron
store node state nodest state item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
check insert updat
updat state statu getstatu item state itemst statu statu
boolean updat exist state getid
string sql updat node state updat sql nodestateupdatesql node state insert sql nodestateinsertsql
byte arrai output stream bytearrayoutputstream
byte arrai output stream bytearrayoutputstream initi buffer size initi buffer size
serial node state
serial serial state
synchron instanc
addition synchron sql statement
execut stmt executestmt sql object byte arrai tobytearrai state node getnodeid string tostr
close bytearrayoutputstream
close
except
string msg fail write node state state node getnodeid
log error msg
item state except itemstateexcept msg
inheritdoc
method share code preparedstat code
execut strictli sequenti method synchron
persist manag instanc synchron
share statement method sychron share
statement synchron
store properti state propertyst state
item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
check insert updat
updat state statu getstatu item state itemst statu statu
boolean updat exist state getid
string sql updat properti state updat sql propertystateupdatesql properti state insert sql propertystateinsertsql
byte arrai output stream bytearrayoutputstream
byte arrai output stream bytearrayoutputstream initi buffer size initi buffer size
serial properti state
serial serial state blob store blobstor
synchron instanc
addition synchron sql statement
execut stmt executestmt sql object byte arrai tobytearrai state properti getpropertyid string tostr
close bytearrayoutputstream
close
except
string msg fail write properti state state properti getpropertyid
log error msg
item state except itemstateexcept msg
inheritdoc
destroi node state nodest state
item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
synchron instanc
addition synchron sql statement
execut stmt executestmt node state delet sql nodestatedeletesql object state node getnodeid string tostr
except
string msg fail delet node state state node getnodeid
log error msg
item state except itemstateexcept msg
inheritdoc
destroi properti state propertyst state
item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
make binari valu blob properli remov
intern internalvalu valu state valu getvalu
valu
valu length
intern internalvalu val valu
val
val type gettyp properti type propertytyp binari
blob file blobfilevalu blob val blobval val blob file getblobfilevalu
delet intern resourc represent blob
blob val blobval delet
remov blobstor
string blob blobid blob store blobstor creat createid state properti getpropertyid
blob store blobstor remov blob blobid
except
log warn fail remov blobstor blob blobid
synchron instanc
addition synchron sql statement
execut stmt executestmt properti state delet sql propertystatedeletesql object state properti getpropertyid string tostr
except
string msg fail delet properti state state properti getpropertyid
log error msg
item state except itemstateexcept msg
inheritdoc
node refer noderefer load node refer nodereferencesid target targetid
item state except nosuchitemstateexcept item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
node refer select sql nodereferenceselectsql
result set resultset
input stream inputstream
statement stmt execut stmt executestmt
node refer select sql nodereferenceselectsql object target targetid string tostr
stmt result set getresultset
item state except nosuchitemstateexcept target targetid string tostr
binari stream getbinarystream
node refer noderefer ref node refer noderefer target targetid
serial deseri ref
ref
except
item state except nosuchitemstateexcept
item state except nosuchitemstateexcept
string msg fail read node refer target targetid
log error msg
item state except itemstateexcept msg
close stream closestream
close result set closeresultset
inheritdoc
method share code preparedstat code
execut strictli sequenti method synchron
persist manag instanc synchron
share statement method sychron share
statement synchron
store node refer noderefer ref
item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
check insert updat
updat exist ref getid
string sql updat node refer updat sql nodereferenceupdatesql node refer insert sql nodereferenceinsertsql
byte arrai output stream bytearrayoutputstream
byte arrai output stream bytearrayoutputstream initi buffer size initi buffer size
serial refer
serial serial ref
synchron instanc
addition synchron sql statement
execut stmt executestmt sql object byte arrai tobytearrai ref getid string tostr
close bytearrayoutputstream
close
except
string msg fail write node refer ref getid
log error msg
item state except itemstateexcept msg
inheritdoc
destroi node refer noderefer ref
item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
synchron instanc
addition synchron sql statement
execut stmt executestmt node refer delet sql nodereferencedeletesql object ref getid string tostr
except
string msg fail delet node refer ref getid
log error msg
item state except itemstateexcept msg
inheritdoc
exist node nodeid item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
node state select exist sql nodestateselectexistsql
result set resultset
statement stmt execut stmt executestmt node state select exist sql nodestateselectexistsql object string tostr
stmt result set getresultset
node state exist result entri
except
string msg fail check exist node state
log error msg
item state except itemstateexcept msg
close result set closeresultset
inheritdoc
exist properti propertyid item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
properti state select exist sql propertystateselectexistsql
result set resultset
statement stmt execut stmt executestmt
properti state select exist sql propertystateselectexistsql object string tostr
stmt result set getresultset
properti state exist result entri
except
string msg fail check exist properti state
log error msg
item state except itemstateexcept msg
close result set closeresultset
inheritdoc
exist node refer nodereferencesid target targetid item state except itemstateexcept
initi
illeg state except illegalstateexcept initi
node refer select exist sql nodereferenceselectexistsql
result set resultset
statement stmt execut stmt executestmt
node refer select exist sql nodereferenceselectexistsql object target targetid string tostr
stmt result set getresultset
refer exist result entri
except
string msg fail check exist node refer
target targetid
log error msg
item state except itemstateexcept msg
close result set closeresultset
misc helper method overrid
initi databas connect persist manag
subclass overrid link getconnect
method default implement call
link getconnect databas connect disabl
autocommit featur
throw except error occur
init connect initconnect except
con connect getconnect
con set auto commit setautocommit
abstract factori method creat databas connect
method call link init pmcontext persist
manag start return connect default
jdbc set link init pmcontext method explicitli
set code autocommit code properti need
note return databas connect entir
lifetim persist manag close
link close link closeconnect connect method
return connect
throw except error occur
connect connect getconnect except
unsupport oper except unsupportedoperationexcept overrid subclass
close databas connect method call
link close close connect acquir
link getconnect persist manag start
default implement call link connect close
method connect subclass overrid
method provid extens databas connect cleanup
param connect databas connect
throw except error occur
close connect closeconnect connect connect except
connect close
establish databas connect method call
link store changelog link executestmt string object
code sqlexcept code encount
return true connect successfulli establish
fals
reestablish connect reestablishconnect
case shut current connect
gracefulli order avoid potenti memori leak
close share prepar statement
iter prepar statement preparedstat valu iter hasnext
prepar statement preparedstat stmt prepar statement preparedstat
stmt
stmt close
sql except sqlexcept
jcr
close connect closeconnect con
except ignor
sleep give databas chanc
restart reconnect attempt
thread sleep sleep reconnect sleep reconnect
interrupt except interruptedexcept ignor
establish connect
init connect initconnect
init prepar statement initpreparedstat
except
log error fail establish connect
reconnect fail
execut sql statement paramet
code sqlexcept code encount
code autoreconnect true code attempt made establish
databas connect execut statement
param sql statement execut
param param paramet set
return code statement code object execut
throw sqlexcept error occur
statement execut stmt executestmt string sql object param
sql except sqlexcept
trial auto reconnect autoreconnect
prepar statement preparedstat stmt prepar statement preparedstat prepar statement preparedstat sql
param length
param size input stream sizedinputstream
size input stream sizedinputstream size input stream sizedinputstream param
stmt set binari stream setbinarystream size getsiz
stmt set object setobject param
stmt execut
reset statement resetstat stmt
stmt
sql except sqlexcept
trial
trial throw
log warn execut fail reconnect messag getmessag
reconnect
reestablish connect reestablishconnect
reconnect succeed check
execut prepar stmt paramet
param length
param size input stream sizedinputstream
size input stream sizedinputstream size input stream sizedinputstream param
consum isconsum
unabl execut prepar stmt
inputstream paramat
'consumed'
throw previou sqlexcept
execut statement
reconnect fail throw previou sqlexcept
reset code preparedstat code clear paramet
warn contain
note method call synchron context
method code preparedstat code instanc
oper thread safe
param stmt code preparedstat code reset
code null code method
reset statement resetstat prepar statement preparedstat stmt
stmt
stmt clear paramet clearparamet
stmt clear warn clearwarn
sql except sqlexcept
log except logexcept fail reset preparedstat
close result set closeresultset result set resultset
close
sql except sqlexcept
log except logexcept fail close resultset
close stream closestream input stream inputstream
close
except ioexcept ignor
close statement closestat statement stmt
stmt
stmt close
sql except sqlexcept
log except logexcept fail close statement
log except logexcept string messag sql except sqlexcept
messag
log error messag
log error reason messag getmessag
log error state code sql state getsqlstat error code geterrorcod
log debug dump
make code schemaobjectprefix code consist
charact allow name target databas illeg
charact escap
throw except error occur
prepar schema object prefix prepareschemaobjectprefix except
databas meta data databasemetadata meta data metadata con meta data getmetadata
string legal char legalchar meta data metadata extra charact getextranamecharact
legal char legalchar abcdefghijklmnopqrstuvwxzy0123456789
string prefix schema object prefix schemaobjectprefix upper case touppercas
string buffer stringbuff escap string buffer stringbuff
prefix length
prefix char charat
legal char legalchar index indexof
escap append
string hex integ hex string tohexstr
escap append char arrai tochararrai hex length
escap append hex
escap append
escap append
schema object prefix schemaobjectprefix escap string tostr
check requir schema object exist creat
exist
throw except error occur
check schema checkschema except
databas meta data databasemetadata meta data metadata con meta data getmetadata
string tabl tablenam schema object prefix schemaobjectprefix node
meta data metadata store lower case identifi storeslowercaseidentifi
tabl tablenam tabl tablenam lower case tolowercas
meta data metadata store upper case identifi storesuppercaseidentifi
tabl tablenam tabl tablenam upper case touppercas
result set resultset meta data metadata tabl gettabl tabl tablenam
schema exist schemaexist
schema exist schemaexist
close
schema exist schemaexist
read ddl resourc
input stream inputstream schema ddl getschemaddl
string msg configur error unknown schema schema
log debug msg
repositori except repositoryexcept msg
buffer reader bufferedread reader buffer reader bufferedread input stream reader inputstreamread
statement stmt con creat statement createstat
string sql reader read line readlin
sql
skip comment empti line
sql start startswith sql length
replac prefix variabl
sql text replac sql schema object prefix variabl schema object prefix variabl schema object prefix schemaobjectprefix
execut sql stmt
stmt execut updat executeupd sql
read sql stmt
sql reader read line readlin
commit
con commit
close stream closestream
close statement closestat stmt
return input stream schema ddl resourc
return input stream schema ddl resourc
input stream inputstream schema ddl getschemaddl
jcr class explicitli getclass
avoid problem subclass packag
databas persist manag databasepersistencemanag resourc stream getresourceasstream schema ddl
build sql statement
build sql statement buildsqlstat
node state insert sql nodestateinsertsql insert
schema object prefix schemaobjectprefix node node data node valu
node state updat sql nodestateupdatesql updat
schema object prefix schemaobjectprefix node set node data node
node state select sql nodestateselectsql select node data
schema object prefix schemaobjectprefix node node
node state select exist sql nodestateselectexistsql select
schema object prefix schemaobjectprefix node node
node state delet sql nodestatedeletesql delet
schema object prefix schemaobjectprefix node node
properti state insert sql propertystateinsertsql insert
schema object prefix schemaobjectprefix prop prop data prop valu
properti state updat sql propertystateupdatesql updat
schema object prefix schemaobjectprefix prop set prop data prop
properti state select sql propertystateselectsql select prop data
schema object prefix schemaobjectprefix prop prop
properti state select exist sql propertystateselectexistsql select
schema object prefix schemaobjectprefix prop prop
properti state delet sql propertystatedeletesql delet
schema object prefix schemaobjectprefix prop prop
node refer insert sql nodereferenceinsertsql insert
schema object prefix schemaobjectprefix ref ref data node valu
node refer updat sql nodereferenceupdatesql updat
schema object prefix schemaobjectprefix ref set ref data node
node refer select sql nodereferenceselectsql select ref data
schema object prefix schemaobjectprefix ref node
node refer select exist sql nodereferenceselectexistsql select
schema object prefix schemaobjectprefix ref node
node refer delet sql nodereferencedeletesql delet
schema object prefix schemaobjectprefix ref node
extern blo externalblob
blob insert sql blobinsertsql insert
schema object prefix schemaobjectprefix binval binval data binval valu
blob updat sql blobupdatesql updat
schema object prefix schemaobjectprefix binval set binval data binval
blob select sql blobselectsql
select binval data
schema object prefix schemaobjectprefix binval binval
blob select exist sql blobselectexistsql
select
schema object prefix schemaobjectprefix binval binval
blob delet sql blobdeletesql delet
schema object prefix schemaobjectprefix binval binval
initi map prepar statement
throw sqlexcept error occur
init prepar statement initpreparedstat sql except sqlexcept
prepar statement preparedstat put
node state insert sql nodestateinsertsql con prepar statement preparestat node state insert sql nodestateinsertsql
prepar statement preparedstat put
node state updat sql nodestateupdatesql con prepar statement preparestat node state updat sql nodestateupdatesql
prepar statement preparedstat put
node state select sql nodestateselectsql con prepar statement preparestat node state select sql nodestateselectsql
prepar statement preparedstat put
node state select exist sql nodestateselectexistsql con prepar statement preparestat node state select exist sql nodestateselectexistsql
prepar statement preparedstat put
node state delet sql nodestatedeletesql con prepar statement preparestat node state delet sql nodestatedeletesql
prepar statement preparedstat put
properti state insert sql propertystateinsertsql con prepar statement preparestat properti state insert sql propertystateinsertsql
prepar statement preparedstat put
properti state updat sql propertystateupdatesql con prepar statement preparestat properti state updat sql propertystateupdatesql
prepar statement preparedstat put
properti state select sql propertystateselectsql con prepar statement preparestat properti state select sql propertystateselectsql
prepar statement preparedstat put
properti state select exist sql propertystateselectexistsql con prepar statement preparestat properti state select exist sql propertystateselectexistsql
prepar statement preparedstat put
properti state delet sql propertystatedeletesql con prepar statement preparestat properti state delet sql propertystatedeletesql
prepar statement preparedstat put
node refer insert sql nodereferenceinsertsql con prepar statement preparestat node refer insert sql nodereferenceinsertsql
prepar statement preparedstat put
node refer updat sql nodereferenceupdatesql con prepar statement preparestat node refer updat sql nodereferenceupdatesql
prepar statement preparedstat put
node refer select sql nodereferenceselectsql con prepar statement preparestat node refer select sql nodereferenceselectsql
prepar statement preparedstat put
node refer select exist sql nodereferenceselectexistsql con prepar statement preparestat node refer select exist sql nodereferenceselectexistsql
prepar statement preparedstat put
node refer delet sql nodereferencedeletesql con prepar statement preparestat node refer delet sql nodereferencedeletesql
extern blo externalblob
prepar statement preparedstat put blob insert sql blobinsertsql con prepar statement preparestat blob insert sql blobinsertsql
prepar statement preparedstat put blob updat sql blobupdatesql con prepar statement preparestat blob updat sql blobupdatesql
prepar statement preparedstat put blob select sql blobselectsql con prepar statement preparestat blob select sql blobselectsql
prepar statement preparedstat put blob select exist sql blobselectexistsql con prepar statement preparestat blob select exist sql blobselectexistsql
prepar statement preparedstat put blob delet sql blobdeletesql con prepar statement preparestat blob delet sql blobdeletesql
class
size input stream sizedinputstream filter input stream filterinputstream
size
consum
size input stream sizedinputstream input stream inputstream size
size size
size getsiz
size
consum isconsum
consum
read except ioexcept
consum
read
skip except ioexcept
consum
skip
read except ioexcept
consum
read
read len except ioexcept
consum
read len
blob store dbblobstor blob store blobstor
inheritdoc
string creat createid properti propertyid index
blobid simpl string concaten index
string buffer stringbuff string buffer stringbuff
append string tostr
append
append index
append
string tostr
inheritdoc
input stream inputstream string blob blobid except
blob select sql blobselectsql
statement stmt execut stmt executestmt blob select sql blobselectsql object blob blobid
result set resultset stmt result set getresultset
close result set closeresultset
except blob blob blobid
input stream inputstream binari stream getbinarystream
databas treat length valu null
return empti inputstream case
close result set closeresultset
byte arrai input stream bytearrayinputstream
return inputstream wrapper order
close resultset stream close
filter input stream filterinputstream
close except ioexcept
close
safe close resultset
close result set closeresultset
inheritdoc
put string blob blobid input stream inputstream size
except
statement stmt execut stmt executestmt blob select exist sql blobselectexistsql object blob blobid
result set resultset stmt result set getresultset
blob exist result entri
exist
close result set closeresultset
string sql exist blob updat sql blobupdatesql blob insert sql blobinsertsql
execut stmt executestmt sql object size input stream sizedinputstream size blob blobid
inheritdoc
remov string blob blobid except
statement stmt execut stmt executestmt blob delet sql blobdeletesql object blob blobid
stmt updat count getupdatecount
